  ...
  double spatialScale)
{
  ...
  hipLaunchKernelGGL(
      ...
      const_cast<const int>(output.numel()), input.data<float>(), rois.data<float>(), static_cast<float>(spatialScale), const_cast<const int>(inputChannels),
      const_cast<const int>(inputHeight), const_cast<const int>(inputWidth), const_cast<const int>(pooledHeight), const_cast<const int>(pooledWidth), output.data<float>(), argmaxes.data<int>());
#else
  RoiPooling2d_forward_kernel<<<grid, block, 0, globalContext().getCurrentCUDAStream()>>>(
  ...
  const Tensor& argmaxes)
{
  ...
  hipLaunchKernelGGL(
      RoiPooling2d_backward_kernel, grid, block, 0, globalContext().getCurrentCUDAStream(),
        const_cast<const int>(gradOutput.numel()), gradOutput.data<float>(), argmaxes.data<int>(), const_cast<const int>(proposals),
        static_cast<float>(spatialScale), const_cast<const int>(inputChannels), const_cast<const int>(inputHeight), const_cast<const int>(inputWidth),
        const_cast<const int>(pooledHeight), const_cast<const int>(pooledWidth), gradInput.data<float>(), rois.data<float>()));
#else
  RoiPooling2d_backward_kernel<<<grid, block, 0, globalContext().getCurrentCUDAStream()>>>(
